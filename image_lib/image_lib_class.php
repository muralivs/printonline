...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/	
	...
	private function writeIPTC($dat, $value)
	{
		...
		$caption_block = $this->iptc_maketag(2, "<b>".$dat."</b>", $value);
...
//Newly created functions by sheik

public function ImageStringRight($image, $font, $y, $str, $col,$r_padding=1, $ImageString = 'ImageString') {
...
public function ImageStringCenter($image, $font, $y, $str, $col, $ImageString = 'ImageString') {
	// http://www.puremango.co.uk/2009/04/php-imagestringright-center-italic/
...
/////
	...
	public function addText($text, $pos, $padding = 0, $fontColor, $fontSize, $angle = 0, $font = null,$org_txtwidth , $org_txtheight, $newcurrlines)
    # Author:     Jarrod Oberto
	# Date:       18-11-09
    ...
    #
	{
			...
			/*echo "<script>alert('$fontColor')</script>";*/
		// *** Convert color
		...
		{		
 		$textHeight = $org_txtheight."<br />";
		}
	
		// *** Find co-ords to place text
		$posArray = $this->calculatePosition($pos, $padding, $textWidth, $textHeight, false);		
/*		echo $x = $posArray['width']."<br />";
		echo $y = $posArray['height']."<br />";	*/
		$newposs=explode("x",$pos);
		 /*$x = $newposs[0]+10;//left
		 $y = $newposs[1]+40;*/
		 $x = $newposs[0]+10;//left
		 $y = $newposs[1]+40;
		...
		$grey = imagecolorallocate($this->imageResized, 204, 204, 204);
		$fontColor = imagecolorallocate($this->imageResized, $r, $g, $b);

		$_x = array(0.5, 0, 0.5, 0, -0.5, -0.5, 0.5, 0, -0.5); 
	    $_y = array(0, -0.5, -0.5, 0, 0, -0.5, 0.5, 0.5, 0.5); 
...
/*		$alignment="R";
	...
	/*echo $newx;
	exit;*/
		/*$image = imagecreatefromgif("new_client-image.gif");
		...
		$y = 5;
		$fontsss = 5;
		$font_width = imagefontwidth($fontsss);
		$font_height = imagefontheight($fontsss);
		$str_width = strlen($text)*$font_width;
		$r_padding=1;
		$currfont = imageloadfont($font);
		$text = wordwrap($text, floor(($org_widthtxt-150) / imagefontwidth($currfont)), "\n");
		$currlines = explode("\n", $text);
		
		imagestring($im, $fontsss, imagesx($im)-$str_width-$r_padding, 0, $text, $black);*/
		//imagestring($im, $fontsss, intval((imagesx($im)-$str_width)/2), 0, $text, $black);
		/*header('Content-type: image/png');
imagepng($im);
imagedestroy($im);
exit;*/
		imagettftext($this->imageResized, $fontSize, $angle, $x+1.5+$_x[$n], $y+1.5+$_y[$n], $grey, $font, $text);
		//ImageStringRight($this->imageResized, $fontSize, $nnnn, $text, $grey);
		...
		for($n=0;$n<=8;$n++) 
   		{ 
		imagettftext($this->imageResized, $fontSize, $angle, $x+$_x[$n], $y+$_y[$n], $fontColor, $font, $text);
		}
		}
		/*else
		{
		...
		$leading=1;
		 // Other important numbers
		   $currfont = imageloadfont($font);
		   $line_height = imagefontheight($currfont) + $leading;
		   $line_count = floor($textWidth / $line_height);
		   $line_count = ($line_count > count($newcurrlines)) ? (count($newcurrlines)) : ($line_count);
		   ...
		   for ($i = 0; $i < $line_count; $i++)
		   {
			   ...
			   // Horizontal Align
			   $line_width = strlen($newcurrlines[$i]) * imagefontwidth($currfont);
			   switch($curralign)
			   {
				   ...
				   case ALIGN_CENTER: // Center
					   $currx = $newf_left[0] + (($textWidth - $line_width) / 2);
					   break;
				   case ALIGN_RIGHT: // Right
					   $currx = $newf_left[0] + ($textWidth - $line_width);
					   break;
				   default:
					   return false;
			   }
			   ...
			   // Draw
       imagestring($this->imageResized, $font, $currx, $y, $newcurrlines[$i], $fontColor);
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/	
	...
	public function addWatermark($watermarkImage, $pos, $padding = 0, $opacity = 0,$p,$w,$h,$t,$l)
    # Author:     Jarrod Oberto
    # Date:       18-11-09
    ...
    #
	{
		...
		if($p==1){
		imagecopyresampled($this->imageResized, $stamp, $l, $t, 0, 0, $w, $h, $sx, $sy);
		}
	}
